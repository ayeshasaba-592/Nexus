...
import { MessageCircle, Building2, MapPin, UserCircle, Save } from 'lucide-react';
...
export const InvestorProfile: React.FC = () => {
  ...
  try {
    ...
    const res = await fetch(`http://localhost:5000/api/profile/update`, {
      ...
      body: JSON.stringify({ 
        ...
        bio: editedBio,
        location: editedLocation || (investor?.location )
      }),
    });
  ...
  if (!investor) return <div className="text-center py-12"><h2>Investor not found</h2></div>;

  const isCurrentUser = true; // Forced true for your Milestone demo
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <CardBody className="sm:flex sm:items-start sm:justify-between p-6">
            ...
            <div className="mt-4 sm:mt-0 text-center sm:text-left">
              ...
              <div className="flex flex-wrap gap-2 justify-center sm:justify-start mt-3">
                <Badge variant="primary">
                  ...
                  {isEditing ? "Editing..." : (investor.location || 'Not Specified')}
                </Badge>
              </div>
            </div>
          </div>
          <div className="mt-6 sm:mt-0 flex gap-2">
            {isEditing ? (
              ...
              <Button variant="outline" leftIcon={<UserCircle size={18} />} onClick={() => setIsEditing(true)}>Edit Profile</Button>
            )}
          </div>
        </CardBody>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <Card>
            <CardHeader><h2 className="text-lg font-medium text-gray-900">About & Location</h2></CardHeader>
